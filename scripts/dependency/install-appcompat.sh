...
jarLocation="$ANDROID_HOME/extras/android/m2repository/com/android/support/appcompat-v7/21.0.3/appcompat-v7-21.0.3.aar"
if [ ! -f "$jarLocation" ]; then
  echo "appcompat-v7 artifact not found!";
  exit 1;
fi

pomLocation="$ANDROID_HOME/extras/android/m2repository/com/android/support/appcompat-v7/21.0.3/appcompat-v7-21.0.3.pom"
...
mvn -q install:install-file -DgroupId=com.android.support -DartifactId=appcompat-v7 \
  -Dversion=21.0.3 -Dpackaging=aar -Dfile="$jarLocation" -DpomFile="$pomLocation"
